    ...
    <FlatList
      ...
      ListHeaderComponent={<View>
        ...
        <View className="rounded-full bg-[#2E2454] px-3 py-2"><Text className="text-xs font-bold tracking-widest text-[#CBB8FF]">REDUX TOOLKIT</Text></View>
          <Pressable accessibilityRole="button" onPress={() => { router.push("/settings" as never); }} style={({ pressed }) => [{ borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surface, borderRadius: 999, paddingHorizontal: 12, paddingVertical: 8 }, { opacity: pressed ? 0.7 : 1, transform: [{ scale: pressed ? 0.96 : 1 }] }]}><Text className="text-sm font-bold text-primary">Aa · чтение</Text></Pressable>
        </View>
      ...
      renderItem={({ item, index }) => <Pressable accessibilityRole="button" onPress={() => { router.push({ pathname: "/volume/[id]", params: { id: item.id } } as never); }} style={({ pressed }) => [{ width: "100%", alignSelf: "stretch", marginBottom: 16, overflow: "hidden", borderRadius: 24, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surface, padding: 20, shadowColor: "#342D72", shadowOpacity: 0.08, shadowRadius: 8, elevation: 2 }, { opacity: pressed ? 0.75 : 1, transform: [{ scale: pressed ? 0.99 : 1 }] }]}>
